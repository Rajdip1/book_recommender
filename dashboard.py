...
text_spliter = CharacterTextSplitter(chunk_size=500, chunk_overlap=50, separator='\n')
docs = text_spliter.split_documents(raw_docs)
embedding_model = HuggingFaceEmbeddings(model_name='sentence-transformers/all-MiniLM-L6-v2')
db_books = Chroma.from_documents(docs, embedding_model)

def retrieve_semantic_recommendations(
        query: str, category: str, tone: str = None, initial_top_k: int = 50, final_top_k: int = 16)->pd.DataFrame:
    recs = db_books.similarity_search(query, k=initial_top_k)
    books_list = [int(rec.page_content.strip('"').split()[0]) for rec in recs]
    books_recs = df[df["isbn13"].isin(books_list)].head(initial_top_k)

    if category != "All":
        book_recs = books_recs[books_recs["simple_categories"] == category].head(final_top_k)
    else:
